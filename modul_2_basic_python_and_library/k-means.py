...
datasetPath = "data\klasering_dataset.txt"
...
def plotClusterResult(listClusterMembers, centroid, iteration, converged):
    ...
    for i in range(n):
        plt.scatter((centroid[i, 0]), (centroid[i, 1]),
                    marker=marker.__next__(), c=col,
                    label="centroid-" + str(i + 1))
    if(converged == 0):
        plt.legend()
        plt.ion()
        plt.show()
...
def kMeans(data, centroidInit):
    ...
    while(True):
        ...
        for i in range(0, data.shape[0]):
            listClusterMember[int(
                clusterMatrix[i])].append(data[i,:])
        # Convert lists of cluster members to numpy arrays for mean calculation
        for i in range(k):
            listClusterMember[i] = np.array(listClusterMember[i])
        ...
        plotClusterResult(listClusterMember,
                          centroidInit, str(iterationCounter), 0)
...
plotClusterResult(clusterResults, centroid,
